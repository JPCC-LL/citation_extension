...
chrome.tabs.query({currentWindow: true, active: true}, function (tabs) {
    ...
    $.ajax({
        url: citationData.webURL, success: function (data) {
            ...
            citationBox.innerHTML = "Cannot generate citation for this page";
    ...
    missingButton.addEventListener('click', function () {
        ...
        authorDiv.innerHTML = "Author <input type=text class='author' value='" + citationData.author + "'>";
        missingSection.appendChild(authorDiv);
        var publisherDiv = document.createElement('div');
        publisherDiv.innerHTML = "Publisher <input type=text class='publisher' value='" + citationData.publisher + "'>";
        missingSection.appendChild(publisherDiv);
        var articleTitleDiv = document.createElement('div');
        articleTitleDiv.innerHTML = "Article Title <input type=text class='article-title' value='" + citationData.articleTitle + "'>";
        missingSection.appendChild(articleTitleDiv);
        var datePublishedDiv = document.createElement('div');
        datePublishedDiv.innerHTML = "Date Published <input type=text class='date-published' value='" + citationData.datePublished + "'>";
        missingSection.appendChild(datePublishedDiv);
        var accessDateDiv = document.createElement('div');
        accessDateDiv.innerHTML = "Access Date <input type=text class='access-date' value='" + citationData.accessDateString + "'>";
        missingSection.appendChild(accessDateDiv);
        var webTitleDiv = document.createElement('div');
        webTitleDiv.innerHTML = "Web Title <input type=text class='web-title' value='" + citationData.webTitle + "'>";
        missingSection.appendChild(webTitleDiv);
        var webUrlDiv = document.createElement('div');
        webUrlDiv.innerHTML = "Web Title <input type=text class='web-url' value='" + citationData.webURL + "'>";
        missingSection.appendChild(webUrlDiv);
        var submitDiv = document.createElement('div');
        submitDiv.innerHTML = "<button type=\"button\" class=\"btn btn-primary\">Submit</button>";
        submitDiv.addEventListener('click', function () {
            ...
            chrome.extension.getBackgroundPage().console.log(authorDiv.getElementsByClassName('author')[0].value);
            ...
            // citationData.accessDate = accessDateDiv.getElementsByClassName('access-date')[0].value;
            citationData.webTitle = webTitleDiv.getElementsByClassName('web-title')[0].value;
            citationData.webURL = webUrlDiv.getElementsByClassName('web-url')[0].value;
            if (selectedCitation == 'MLA') {
                citationBox.innerText = MLACitation(citationData);
            } else if (selectedCitation == 'APA') {
                citationBox.innerText = APACitation(citationData);
            } else {
                citationBox.innerText = ChicagoCitation(citationData);
            }
        });
        missingSection.appendChild(submitDiv);
    });
});
