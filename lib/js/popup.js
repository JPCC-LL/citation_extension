...
chrome.tabs.query({currentWindow: true, active: true}, function (tabs) {
    ...
    editButton.addEventListener('click', function () {
        ...
        var generateInnerHtml = function (id, classValue, textValue) {
            return id + "<input type=text class='" + classValue +
                    "' style='width: 100%' value='" + textValue + "'>";
        };

        //Adds the Divs and buttons to be displayed and edited
        var authorDiv = document.createElement('div');
        authorDiv.innerHTML = generateInnerHtml('Author', 'author', citationData.author);
        missingSection.appendChild(authorDiv);
        var publisherDiv = document.createElement('div');
        publisherDiv.innerHTML = generateInnerHtml('Publisher', 'publisher', citationData.publisher);
        missingSection.appendChild(publisherDiv);
        var articleTitleDiv = document.createElement('div');
        articleTitleDiv.innerHTML = generateInnerHtml('Article Title', 'article-title', citationData.articleTitle);
        missingSection.appendChild(articleTitleDiv);
        var webTitleDiv = document.createElement('div');
        webTitleDiv.innerHTML = generateInnerHtml('Web Title', 'web-title', citationData.webTitle);
        missingSection.appendChild(webTitleDiv);
        var datePublishedDiv = document.createElement('div');
        datePublishedDiv.innerHTML = generateInnerHtml('Date Published', 'date-published', citationData.datePublished);
