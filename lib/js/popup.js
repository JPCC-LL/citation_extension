/**
 * Generates an a radio button group
 *
 * @param value Input value
 * @param text Text to be displayed
 * @param selected Boolean is radio button is selected
 * @returns {string} String of radio button groups
 */
function generateRadioInnerHtml(value, text, selected) {
    if (selected) {
        return "<input type='radio' name='page' value='" + value + "' style='margin: auto' checked>" + text;
    } else {
        return "<input type='radio' name='page' value='" + value + "' style='margin: auto'>" + text;
    }
}

//Radio Button group (only exists for MLA). Only one is needed to exist in the extension.
var articleSelectionDiv = document.createElement('div');
articleSelectionDiv.style.textAlign = 'center';
...
chrome.tabs.query({currentWindow: true, active: true}, function (tabs) {
    ...
    $.ajax({
        url: citationData.webURL, success: function (data) {
            ...
            //Function called when user switches from Citation Style (MLA->Chicago, Chicago->APA, etc)
            var preCitationSelection = function () {
                missingSection.innerText = "";
    ...
    editButton.addEventListener('click', function () {
        ...
        //If MLA is selected it appends the radio button group.
        //Otherwise, it sets article type to 'article' (it is the only type that exists for 'APA' & 'Chicago'
        if (selectedCitation == 'MLA') {
        ...
        /**
         * Generates an input html for the user to edit values
         *
         * @param id Description of the edit text
         * @param classValue html text value
         * @param textValue Value currently stored
         * @returns {string}
         */
        ...
        submitDiv.addEventListener('click', function () {
            ...
            switch (currentSelection) {
                case 'article':
                    ...
                    if (tagNameArray[0].value != "undefined") {
                        citationData.author = tagNameArray[0].value;
                    }
                    if (tagNameArray[1].value != "undefined") {
                        citationData.publisher = tagNameArray[1].value;
                    }
                    if (tagNameArray[2].value != "undefined") {
                        citationData.articleTitle = tagNameArray[2].value;
                    }
                    if (tagNameArray[3].value != "undefined") {
                        citationData.webTitle = tagNameArray[3].value;
                    }
                    if (tagNameArray[4].value != "undefined") {
                        citationData.datePublished = tagNameArray[4].value;
                    }
                    break;
                case 'image':
                    tagNameArray = imageDiv.getElementsByTagName('input');
                    if (tagNameArray[0].value != "undefined") {
                        citationData.artist = tagNameArray[0].value;
                    }
                    if (tagNameArray[1].value != "undefined") {
                        citationData.artwork = tagNameArray[1].value;
                    }
                    if (tagNameArray[2].value != "undefined") {
                        citationData.location = tagNameArray[2].value;
                    }
                    if (tagNameArray[3].value != "undefined") {
                        citationData.institution = tagNameArray[3].value;
                    }
                    break;
                case 'email':
                    ...
                    if (tagNameArray[1].value != "undefined") {
                        citationData.subject = tagNameArray[1].value;
                    }
                    if (tagNameArray[2].value != "undefined") {
                        citationData.receiver = tagNameArray[2].value;
                    }
                    break;
                case 'tweet':
                    ...
                    if (tagNameArray[1].value != "undefined") {
                        citationData.twitterUsername = tagNameArray[1].value;
                    }
                    if (tagNameArray[2].value != "undefined") {
                        citationData.tweet = tagNameArray[2].value;
                    }
                    if (tagNameArray[3].value != "undefined") {
                        citationData.datetime = tagNameArray[3].value;
                    }
                    break;
                case 'video':
                    ...
                    if (tagNameArray[1].value != "undefined") {
                        citationData.videoname = tagNameArray[1].value;
                    }
                    if (tagNameArray[2].value != "undefined") {
                        citationData.mediaType = tagNameArray[2].value;
                    }
                    if (tagNameArray[3].value != "undefined") {
                        citationData.webTitle = tagNameArray[3].value;
                    }
        ...
        hideDiv.addEventListener('click', function () {
            ...
            //It has the same functionality as the preCitationSelection function, except doesn't hide the section
        ...
        //Each selection must check if the current one doesn't exist. No need to re-add existing selection
        ...
        //Iterates through the radio buttons and registers an event listener
